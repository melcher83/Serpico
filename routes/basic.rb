...
post '/login' do
  ...
  if user && (user.auth_type == 'Local')
    ...
    if config_options['ldap'].to_s == 'true'
      ...
      ldap = Net::LDAP.new host: (config_options['ldap_dc']).to_s, port: 636, encryption: :simple_tls, auth: {method: :simple, username: user, password: params[:password], base: basen}
